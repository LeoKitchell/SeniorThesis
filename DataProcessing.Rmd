...
for (variable in file_list) {
  if(grepl("reviews",variable,fixed=TRUE)){
      ...
      review_data2 <- rbind(review_data2,read.csv(paste(path,variable,sep = ""),encoding = "UTF-16LE"))
  } 
}

file <- unzip(paste(path,"reviews_NewYorkCity.csv.zip",sep=""),exdir = getwd())
filter(review_data,review_data$comments)
...
sampleSize <- 800#nrow(combinedData)
...
sparse <- removeSparseTerms(frequencies, 0.99) #removes words which show up very few times across reviews.
...
trainSparse
filter(trainSparse,trainSparse$baã.o>0)
```
