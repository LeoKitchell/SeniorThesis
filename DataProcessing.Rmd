...
speedFactor <- .2         #Value between 0 and 1. Percent of tracts to use in training and testing set. When running full model set to 1. 
...
while (ii <= length(trainSparseListTFIDF)) {
      rangerLargeModelsStructuredTFIDF[[ii]] <- caret::train(gentrificationChange ~ .,
                                      data = tSparseListStructuredTFIDF[[ii]],
                                      method = "ranger",
                                      importance = "impurity",
                                      num.trees = 200)
