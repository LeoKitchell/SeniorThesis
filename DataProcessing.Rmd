...
sampleSize <- 10#nrow(combinedData)
...
#corpus <- Corpus(VectorSource(tractData$comments)) this line takes a long time to run but only needs to be run once
...
sparsities <- c(.5,.6,.7,.8,.9,.95)

sparse <- list()
tSparse <- list()
ii<-1
while (ii < length(sparsities)+1) {
  sparse[[ii]] <- removeSparseTerms(frequencies, sparsities[ii])
  tSparse[[ii]] <- as.data.frame(as.matrix(sparse[[ii]]))
  colnames(tSparse[[ii]]) <- make.names(colnames(tSparse[[ii]]))
  ii <- ii+1
}
...
```{r}
predictorData <- subset(tractData,select = c(gentrificationChange,totalPop.2014,popGrowth,review_scores_rating,
...
```{r}
index <- sample(seq_len(nrow(tSparse)), size = nrow(tSparse)*.01) #splits 70% of the data into a trainng set and 30% into a test set.
...
RF_model = randomForest(gentrificationChange ~ ., data=trainSparse,na.action = na.omit, ntree = 2)
...
RF_model_MultiThread3 <- train(gentrificationChange ~ ., data =trainSparse,
                     method = "ranger",
                     importance = 'impurity',
                     num.trees = 15)

predictRF_Multi = predict(RF_model_MultiThread3, newdata=testSparse)


varImp(RF_model_MultiThread3)
...
RF_model_MultiThread2 <- train(gentrificationChange ~ ., data =trainSparse,
                     method = "ranger",
                     num.trees = 900)

predictRF_Mult2i = predict(RF_model_MultiThread2, newdata=testSparse)
