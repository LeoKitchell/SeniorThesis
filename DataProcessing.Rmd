...
tCensusData <- censusDataWithDistributionsWide %>% subset(select = c("tractFIPS","NAME.2014","totalPop.2014","popGrowth","gentrificationChange"))
...
corpus <- Corpus(VectorSource(combinedData[1:500,]$comments))
corpus <- tm_map(corpus, PlainTextDocument)
corpus <- tm_map(corpus, tolower)
corpus <- tm_map(corpus, removePunctuation)
corpus <- tm_map(corpus, removeWords, c("airbnb",stopwords("english")))  #remove stopwords and add "airbnb" to stopword list
...
sparse <- removeSparseTerms(frequencies, 0.995) #removes words which show up very few times across reviews.
...
```{r}
predictorData <- subset(combinedData[1:500,],select = c(gentrificationChange))#,totalPop.2014,popGrowth))
tSparse <- cbind(predictorData,tSparse)

tSparse <- tSparse[,!duplicated(colnames(tSparse))] #removing columns with duplicate names
sum(!complete.cases(tSparse)) #number of rows with NAs.
```

## Splitting Data into Testing and Training Sets
```{r}
index <- sample(seq_len(nrow(tSparse)), size = nrow(tSparse)*.7)
...
mean(tSparse$gentrificationChange)  #0.254 is the baseline accuracy
