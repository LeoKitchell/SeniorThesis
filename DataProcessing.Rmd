...
ii<-1
rangerResults <- data.frame(matrix(ncol = 6,nrow = 1))
colnames(rangerResults) <- c("results","sparsity","rsquared","MSE","NumTrees","mTry")
rangerResults[1,] <- 1
while (ii <= length(rangerLargeModels)) {
  row <- c("RSquared",sevenLevelSparsity[ii],rangerLargeModels[[ii]]$finalModel$r.squared,
           rangerLargeModels[[ii]]$finalModel$prediction.error,rangerLargeModels[[ii]]$finalModel$num.trees,
           rangerLargeModels[[ii]]$finalModel$mtry)

  rangerResults <- rbind(rangerResults,row)
}
