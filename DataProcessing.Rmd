...
ii<-1
while (ii <= length(trainSparseList)) {
      rangerLargeModelsStructured[[ii]] <- caret::train(gentrificationChange ~ .,
                                      data = trainSparseList[[ii]],
                                      method = "ranger",
                                      importance = "impurity",
                                      num.trees = 200)
...
save(list = c("rangerLargeModelsStructured","impPlotsLargeRangerStructured","predictLargeRangersStructured"),file = "./Data/OtherData/largeRangerResultsStructured.RData")
...
ii<-1
while (ii <= length(trainSparseListTFIDF)) {
      rangerLargeModelsStructuredTFIDF[[ii]] <- caret::train(gentrificationChange ~ .,
                                      data = trainSparseListTFIDF[[ii]],
                                      method = "ranger",
                                      importance = "impurity",
                                      num.trees = 200)
...
save(list = c("rangerLargeModelsStructuredTFIDF","impPlotsLargeRangerStructuredTFIDF","predictLargeRangersStructuredTFIDF"),file = "./Data/OtherData/largeRangerResultsStructuredTFIDF.RData")
...
speedTrainSparseListStructured <- lapply(speedTrainSparseList, "[", c(1:9))
speedTestSparseListStructured  <- lapply(speedTestSparseList,"[", c(1:9))

ii<-1
while (ii <= length(speedTrainSparseList)) {
      rangerSmallModelsStructured[[ii]] <- caret::train(gentrificationChange ~ .,
                                      data = speedTrainSparseListStructured[[ii]],
                                      method = "ranger",
                                      importance = "impurity",
                                      num.trees = 15)
      predictSmallRangersStructured[[ii]] <- predict(rangerSmallModels[[ii]],newdata = speedTestSparseListStructured[[ii]])
      impPlotsSmallRangerStructured[[ii]] <- plot(varImp(rangerSmallModels[[ii]]),top = 15)
      print(paste("Model",ii,"finished at",Sys.time()))
      ii<-ii+1
}

save(list = c("rangerSmallModelsStructured","impPlotsSmallRangerStructured","predictSmallRangersStructured"),file = "./Data/OtherData/smallRangerResultsStructured.RData")

testMSESmallRangerStructured <- double()
ii<-1
while (ii <= length(rangerSmallModelsStructured)) {
  testMSESmallRangerStructured[ii] <- sum((predictSmallRangersStructured[[ii]]-testData[[3]])**2)/length(testData[[3]])
  ii <- ii+1
}
testMSESmallRangerStructured
impPlotsSmallRangerStructured
...
ii<-1
rangerResults <- data.frame(matrix(ncol = 7,nrow = 1))
colnames(rangerResults) <- c("trainDataSize","sparsity","rsquared","MSE","NumTrees","mTry","minNodeSize")
rangerResults[1,] <- 1
while (ii <= length(rangerLargeModels)) {
  row <- c(rangerLargeModels[[ii]]$finalModel$num.samples,sevenLevelSparsity[ii],rangerLargeModels[[ii]]$finalModel$r.squared,
           rangerLargeModels[[ii]]$finalModel$prediction.error,rangerLargeModels[[ii]]$finalModel$num.trees,
           rangerLargeModels[[ii]]$finalModel$mtry,rangerLargeModels[[ii]]$finalModel$min.node.size)
