...
head(tractDataNoText)
tbl_summary(tractDataNoText)
tractDataNoText %>% select(-tractFIPS) %>% tbl_summary()
...
ggplot(data = rangerResults, aes(x = sparsity, y = as.double(MSE), group = modelName)) +
      geom_line(aes(color = modelName)) + labs(title = "Model Accuracy by Sparsity and NTrees") + ylab("Mean Squared Error")

ggplot(data = rangerResults, aes(x = sparsity, y = as.double(rsquared), group = modelName)) +
      geom_line(aes(color = modelName)) + labs(title = "RSquared by Sparsity and NTrees") + ylab("R Squared")
```
## Tuning Mtry
```{r}
...
tunedExtraLargeRanger <- tuneMtryFast( gentrificationChange ~ .,
                                       ...
                                       num.trees = 400,
                                       plot = TRUE,
                                       mtryStart = 913, #2690 provides a similar prediction error
                                       stepFactor = 1.0025,
                                       improve = 0.0001,
                                       trace = TRUE)
```

## Training Candidate Final Models
```{r}
runtime <- list()
mtryOptimal <- 913
sparseOptimal <- 6
...
## Combining Results of Candidate Final Models
```{r}
...
#pink sweats, erin may ? hip hop. 

optimalRangerResults <- rbind(optimalRangerResults,row1,row2,row3, row4,row5)
...
optimalRangerResultsTall <- gather(optimalRangerResults,key = measure, value = values, c("rsquared","MSE")) 
optimalRangerResultsTall
```

## Comparing Candidate Final Models
```{r}
scalingCoef <- floor(mean(as.double(optimalRangerResults$rsquared)/as.double(optimalRangerResults$MSE)))
scalingCoef <- .065
colors <- c("rsquared" = "red", "MSE" = "blue")

ggplot(data = optimalRangerResults, aes(x=sparsity, group=1)) +
  geom_line( aes(y=as.double(MSE)/scalingCoef, color = "MSE")) + 
  geom_line( aes(y=as.double(rsquared), color = "rsquared")) +
  scale_color_manual(values = colors) +
  labs(x = "Model Sparsity",color = "Legend")+
  scale_y_continuous(
