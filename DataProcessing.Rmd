...
sampleSize <- 500#nrow(combinedData)
...
sparse <- removeSparseTerms(frequencies, 0.95) #removes words which show up very few times across reviews.
...
```{r}
index <- sample(seq_len(nrow(tSparse)), size = nrow(tSparse)*.70) #splits 70% of the data into a trainng set and 30% into a test set.
...
RF_model_MultiThread <- train(gentrificationChange ~ ., data =trainSparse,
                     method = "ranger",
                     num.trees = 300)

predictRF_Multi = predict(RF_model_MultiThread, newdata=testSparse)
