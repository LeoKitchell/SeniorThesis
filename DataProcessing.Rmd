...
sampleSize <- 200
combinedData1 <-combinedData[sample(nrow(combinedData), sampleSize), ]
corpus <- Corpus(VectorSource(combinedData1$comments))
...
```{r}
predictorData <- subset(combinedData1,select = c(gentrificationChange))#,totalPop.2014,popGrowth))
...
mean(tSparse$gentrificationChange)  #baseline accuracy??
