...
testdata <- getCensus(name = nameACS,vintage = vintage, vars = testVars, regionin = regionin, region = region)

censusData <- data.frame()
for (fipsCode in includedStates$State_FIPS) {
  state = paste("state:",fipsCode,"+county:*",sep = "")
  print(paste("Now pulling census data for Geography:",state))
  stateData <- getCensus(name = nameACS,vintage = vintage, vars = censusVars, regionin = state, region = region)
