...
censusVars <- c(locationName,totalPop,medHHInc,medRentAsPctInc,rawRent,medGrossRent,educationVars)
```
...
for (year in vintage) {
  for (city in uniqueMSAs$CBSA.Code) {
    ...
    rentDist <- ecdf(cityCensusData$medRentAsPctInc)
    cityCensusData$pctilemedRentAsPctInc <- rentDist(cityCensusData$medRentAsPctInc)
    
    censusDataWithDistributions <- rbind(censusDataWithDistributions,cityCensusData)
    print(paste(year,city))
  }
}

#testing to see if the code worked
a<- censusDataWithDistributions %>% filter(vintage==year,CBSA.Code==city) %>% subset(select=c("tract","NAME","medHHInc","CBSA.Code","pctilemedHHInc"))
testDist <- ecdf(a$medHHInc)
head(a)
hist(a$medHHInc)
testDist(86000)
hist(censusData$medRentAsPctInc)
hist(censusData$medHHInc)
filter(censusData,medHHInc<20000,vintage==2019,state==53)
hist(censusData$B19013_001E) #negative vals
hist(censusData$B25071_001E) #negative vals
hist(censusData$B25064_001E) #negative vals
hist(censusData$B25056_001E) #no outliers
```
