...
listing_data <-listing_data[-1,] #removes first blank row.
...
#for (year in vintage) {
  for (listing in 124000:nrow(listing_data[])) {
  lat = listing_data$latitude[listing]
    long = listing_data$longitude[listing]
    url <- paste("https://geo.fcc.gov/api/census/block/find?latitude=",lat,"&longitude=",long,"&censusYear=",2000,"&format=json",sep = "")
    response = GET(url)
    blockData <- response$content%>%rawToChar()%>%fromJSON()
    
    if(is.null(blockData$Block$FIPS)){
        listing_data$fullFIPS2000[listing]<-NA
        listing_data$tractFIPS2000[listing]<-NA
    }else{
        listing_data$fullFIPS2000[listing] <- blockData$Block$FIPS
        listing_data$tractFIPS2000[listing] <- substr(listing_data$fullFIPS2000[listing],1,11)
    }
    if(listing%%1000==0) {
