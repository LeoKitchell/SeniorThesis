...
censusDataWithDistributionsWide %>% select(NAME.2014,state.2014,county.2014,tractFIPS,gentrificationIndex.2014,gentrificationIndex.2019,gentrificationChange) %>% arrange(desc(gentrificationChange)) %>% filter(state.2014==53)
```
...
## Interactive Maps - Unfinished
```{r,eval=FALSE}
pal <- colorQuantile("Greens", NULL, n = 6)
popup <- paste0("Gentrification Change: ", as.character(cityCensusData$gentrificationChange))
leaflet() %>%
