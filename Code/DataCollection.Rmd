...
```{r}
listing_data <- read.csv(file = "./Data/OtherData/listing_data_11_29.csv.gz",colClasses = c("fullFIPS2000"="character","fullFIPS2010"="character",
...
## Specification
The specification of gentrification which will be used in this project is heavily inspired by the work of Freeman (2005) and Bates (2013). To compose our gentrification measure we will use Census tract level data on median household income, educational attainment, and median rent cost.
...
#Education Variables.
  ...
  bachelorsPop <- "B15003_022E" #Number of respondents 25 Years and Over with a bachelors degree as their highest level of education
  mastersPop <-   "B15003_023E" #Number of respondents 25 Years and Over with a masters degree as their highest level of education
  professionalPop <-"B15003_024E" #Number of respondents 25 Years and Over with a professional degree as their highest level of education
  doctoratePop <-"B15003_025E" #Number of respondents 25 Years and Over with a doctorate degree as their highest level of education
...
```{r}
# Add key to .Renviron
key = "be936e1698dd7dfd657546a6a7088c6306f4e54f"  #Personalized key should be created for new users. 
...
```{r}
cbsasRaw <- read.csv("./Data/OtherData/CBSAs_March_2020.csv",header = TRUE,colClasses=c("FIPS.State.Code"="character","FIPS.County.Code"="character"))  #Census delineation file downloaded from "https://www.census.gov/geographies/reference-files/time-series/demo/metro-micro/delineation-files.html" with first two rows deleted and saved as csv.
...
msaMappingData <- subset(censusDataWithDistributionsWideGeoms, select = c("GEOID","NAME.2014","CBSA.Title.2014","CBSA.Code.2014"))
save(msaMappingData,uniqueMSAs,file = "Data/OtherData/msaMappingData.RData")
```
...
runtime$mapping <- Sys.time() - runtime$mapping
```
## Histograms of Gentrification in Gentrifiable Tracts
```{r}
library(ggplot2)
gentrifiableHistogram <- ggplot(data = gentrifiableCensusDataWithDistributionsWide, aes(gentrificationChange)) +
                         geom_histogram() +
                         labs(x = "Change in Gentrification Index", y = "# of Tracts",title = "Gentrification Change Index") +
                         theme(axis.title.x = element_text(size=15), axis.title.y = element_text(size = 15))
gentrification2014Histogram <-  ggplot(data = gentrifiableCensusDataWithDistributionsWide, aes(gentrificationIndex.2014)) +
                         geom_histogram() +
                         labs(x = "Gentrification Index", y = "# of Tracts",title = "Gentrification Index in 2014")+
                          theme(axis.title.x = element_text(size=15), axis.title.y = element_text(size = 15))
gentrification2019Histogram <-  ggplot(data = gentrifiableCensusDataWithDistributionsWide, aes(gentrificationIndex.2019)) +
                         geom_histogram() +
                         labs(x = "Gentrification Index", y = "# of Tracts",title = "Gentrification Index in 2019")+
                         theme(axis.title.x = element_text(size=15), axis.title.y = element_text(size = 15))

gentrifiableHistogram
ggsave(filename = "./Writing/Figures/GentrifiableHistogram.PNG",width = 4.5 , height = 3)
gentrification2014Histogram
ggsave(filename = "./Writing/Figures/GentrificationHistogram2014.PNG",width = 4.5 , height = 3)
gentrification2019Histogram
ggsave(filename = "./Writing/Figures/GentrificationHistogram2019.PNG",width = 4.5 , height = 3)
```
